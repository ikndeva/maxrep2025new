...
\begin{abstract}
  ...
  This work extends the classic problem of constructing a suffix tree from a Suffix Array in the context of output-sensitive computation on a repetition-aware text index.
...
%%%%
store all of $O(n^2)$ factors in linear or less words of space supporting a similar set of operations efficiently, the latter compresses the former by reducing its space from $O(n)$ to $O({e_R(S) + e_L(S)})$, where ${e_R(S) + e_L(S)}$ is a parameter that measures the compressibility of a string. The parameter ${e_R(S) + e_L(S)}$ is defined as the sum of the the numbers $e_R$ and $e_L$ of all right- and left-extensions of the maximal repeats of the string.
...
In an application senario in, e.g., a human genome repository, the structure $\sig I$ will be constructed once, reside on memory using $O(n)$ space, and be used many times for the generation of the CDAWG in $O({e_R(S) + e_L(S)})$ time and working space per execution, and other purposes, such as the standard string search.
For the preprocessing, all of the above structures occupy $O(n)$ words of space, and can be constructed in $O(n)$ time from a string $S$ over an integer alphabet in preprocessing (for details, see the textbook or survey article by Navarro~\cite{navarro2016cds:book,navarro2021indexing:ii}).
...
\subsection{Results}
%%% 
Under the problem setting explained above, we then show the main result of this paper. 
...
From \cref{thm:main:index:cdawg} above, the proposed algorithm improves the time and space complexity $\Theta(n)$ of the straightforward algorithms via suffix tree construction to asymptotic time and space complexity $O(e_R(S)+e_L(S)$ for classes of repetitive texts with $\max\set{e_R(S), e_L(S)} = o(n)$.
%%
...
%%%
Recently, repetition-aware indexes have attracted much attention. A recent compressed index, called \textit{r-index}, proposed by Gagie, Navarro, and Prezza~\cite{gagie:navarro:prezza2020fully}, as well as its latest version~\cite{nishimoto2022optimaltime:rlbwt:construction}, that occuppies $O(r(S)\polylog(n))$ space with supporting the similar functionality of the afoementioned classic array index $\sig I$ and $S$. 
Since our algorithm makes read-only probes to a limited number of cells of $\sig I$ and $S$, the next theorem immediately follows from \cref{thm:main:index:cdawg} above. 
...
\end{minipage}
%% \vspace{.5\baselineskip}
\caption{An example of the \LPTrm-tree $T$ of a string $S = aabaababb\daller$ in \cref{fig:example:problem}.
...
For any factor $u \in \Fac$, we define the \textit{right-closure} of $u$, denoted $\rext{u}$, to be the string obtained from $u$ by maximally extending $u$ rightwards while the occurrence of the resulting factor does not change, i.e., $\Spos(\rext{u}) = \Spos(u)$ holds.
...
Blumer \textit{et al.}~\cite{blumer1987complete} showed the properties of the CDAWG $G$ of $S$, which is the minimum state DFA of $\Suf$ with path-compression. We define the \textit{end-equivalence relation} $\equiv_{epos}$ over $\Fac$ by $x \equiv_{epos} y \iff\Epos(x) = \Epos(y)$.
%%%
Each node $\underline u$ of $G$ with $u \in \Fac$ represents an equivalence class, denoted by $[\rext{u}]^S_{epos} \subseteq \Fac$, of factors of $S$ with respect to $\equiv^S_{epos}$. That is, $\underline u := [\rext{u}]^S_{epos} = \sete{ v \in \Fac \mid u \equiv_{epos} v}$.
%% Then, each node $\underline u$ of $G$ represents the equivalence class $[\rext{u}]^S_{epos} \subseteq \Fac$ of factors of $S$ with a representative $u \in \Fac$ w.r.t.~$\equiv^S_{epos}$, namely, $\underline u := [\rext{u}]^S_{epos} = \sete{ v \in \Fac \mid u \equiv_{epos} v}$.
%%%
...
\begin{lemma}[Belazzougui \textit{et al.}~\cite{belazzougui:nunial:gagie:prezza:raffinot2015composite}]\label{lem:lpt:edge:classification}
  The arc subsets $\sig E$ and $\sig F$ of $\LPT$ correspond to the sets of all primary arcs and all non-primary arcs in $G = \CDAWG(S)$. That is, for all pair of factors $u, v\in \Fac$, the arc $f = (\underline u, \underline v)$ in $\sig E$ and the corresponding arc $f' = ([\rext{u}]^S_L, [\rext{v}]^S_L)$ satisfies the conditions (1) and (2) below: 
\begin{enumerate}[(1)]  
...
\item $\islb(u) \in \set{\tt true, false}$: The flag indicating whether $u$ is right-branching in $S$, i.e., $|\LChar(u)|\ge 2$. 
...
the following structures computed during the computation: 
\begin{itemize}
