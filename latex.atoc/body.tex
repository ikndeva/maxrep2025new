...
As a read-only input, we assume an index structure $\sig I$ consisting of the following structures preprocessed from $S$ beforehand, and given to an algorithm:
\begin{itemize}
\item the suffix array $SA$,
\item the inverse suffix array $ISA$,
\item the longest common prefix array $LCP$, 
\end{itemize}
where the LCP array is augmented by the data structure supporting  
the range-minima query (RMQ) on $LCP$: Given any ssubrange $i..j \subseteq 1..n$, returns the minimum of the LCP-values $\set{LCP[k] \mid 1\le k\le j}$ within $i..j$.
...
Finally, we use as measures of repetitiveness the numbers of all maximal repeats, all right- and all left-extensions of maximal repeats, respectively, denote by $\mu(S)$, $e_R(S)$, and $e_L(S)$.
...
\begin{corollary}\label{cor:main:index:maxrep}
  We can enumerate the set $\MR(S)$ of all distinct maximal repeats a string $S$ of length $n$ over an integer alphabet $\Sigma$ in $O(e_R(S)+e_L(S) + \mu(S)\log \mu(S))$ time and $O(\sigma \log n)$ working space using $O(e_R(S) + e_L(S))$ probes to the afoementioned text index $\sig I$ and the string $S$. 
  %% the afoementioned text index $\sig I = (\SA, \ISA, \LCP)$ and the read-only$S$ of $O(n)$ total size. 
\end{corollary}
